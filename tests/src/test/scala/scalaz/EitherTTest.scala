...
object EitherTTest extends SpecLite {
  ...
  object instances {
    def functor[F[_] : Functor, A] = Functor[EitherT[A, F, ?]]
    def bindRec[F[_] : Monad: BindRec, A] = BindRec[EitherT[A, F, ?]]
    def monad[F[_] : Monad, A] = Monad[EitherT[A, F, ?]]
    def plus[F[_] : Monad, A: Semigroup] = Plus[EitherT[A, F, ?]]
    def monadPlus[F[_] : Monad, A: Monoid] = MonadPlus[EitherT[A, F, ?]]
    def foldable[F[_] : Foldable, A] = Foldable[EitherT[A, F, ?]]
    def traverse[F[_] : Traverse, A] = Traverse[EitherT[A, F, ?]]
    ...
    def functor[F[_] : BindRec, A] = Functor[EitherT[A, F, ?]]
    def functor[F[_] : Monad, A: Monoid] = Functor[EitherT[A, F, ?]]
    def functor[F[_] : Monad : BindRec, A: Monoid] = Functor[EitherT[A, F, ?]]
    def apply[F[_] : Monad, A: Monoid] = Apply[EitherT[A, F, ?]]
    def monad[F[_] : Monad, A: Monoid] = Monad[EitherT[A, F, ?]]
    def plus[F[_] : Monad, A: Monoid] = Plus[EitherT[A, F, ?]]
    def foldable[F[_] : Traverse, A] = Foldable[EitherT[A, F, ?]]
    def bifunctor[F[_] : Traverse] = Bifunctor[EitherT[?, F, ?]]
    def bifoldable[F[_] : Traverse] = Bifoldable[EitherT[?, F, ?]]
    def monadError[F[_] : Monad, A] = MonadError[EitherT[A, F, ?], A]
    def nondeterminism[F[_] : Nondeterminism, A] = Nondeterminism[EitherT[A, F, ?]]
    def nondeterminismMonad[F[_] : Nondeterminism, A] = Monad[EitherT[A, F, ?]]
    def nondeterminismFunctor[F[_] : Nondeterminism: BindRec: Traverse, A] = Functor[EitherT[A, F, ?]]
    def nondeterminismMonad[F[_] : Nondeterminism: BindRec: Traverse, A] = Monad[EitherT[A, F, ?]]
  }
